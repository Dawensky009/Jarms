...
const HERO_REEL = "/videos/the-blind-eye.mp4";
...
function HeroReel({ onOpen }: { onOpen: () => void }) {
  ...
  return (
    <button
      ...
      className="group relative block aspect-video w-full cursor-pointer overflow-hidden rounded-4xl border border-ink/10 bg-night shadow-2xl shadow-ink/20 transition-[transform,box-shadow] duration-300 ease-out-strong hover:-translate-y-1 hover:shadow-[0_45px_90px_-35px_rgba(20,22,29,0.5)] active:scale-[0.99]"
    >
      <video
        ref={ref}
        src={HERO_REEL}
      ...
      <span className="absolute inset-0 flex items-center justify-center">
        <span className="flex h-16 w-16 items-center justify-center rounded-full bg-white/90 text-ink shadow-xl backdrop-blur transition-transform duration-300 ease-out-strong group-hover:scale-110">
          <Play className="ml-1 h-6 w-6 fill-current" />
        </span>
      </span>

      <span className="absolute bottom-4 left-4 inline-flex items-center gap-2 rounded-full bg-night/55 px-3 py-1.5 text-xs font-medium text-white backdrop-blur">
...
export function Hero() {
  ...
  return (
    <section
      id="top"
      className="grain relative overflow-hidden bg-white pb-12 pt-28 sm:pb-16 sm:pt-36"
    >
        ...
        <div className="grid items-center gap-12 lg:grid-cols-[1.04fr_0.96fr] lg:gap-14">
          ...
          <div className="text-center lg:text-left">
            ...
            <motion.p
              ...
              className="mx-auto mt-6 max-w-xl text-base text-ink-muted sm:text-lg lg:mx-0"
            >
            ...
            <motion.div
              ...
              className="mt-8 flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-sm text-ink-muted lg:justify-start"
            >
      ...
      <VideoModal
        project={
          open
            ? { title: "Jarms Showreel", client: "Selected work", videoSrc: HERO_REEL }
            : null
        }
