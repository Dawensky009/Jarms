...
export function Faq() {
  ...
  return (
    <section id="faq" className="bg-night-soft">
      <div className="container-px mx-auto max-w-container py-20 sm:py-28">
        <div className="grid gap-12 lg:grid-cols-[0.82fr_1.18fr]">
          <Reveal>
            ...
            <div className="mt-8 flex items-start gap-3 rounded-3xl border border-gold/30 bg-night p-5">
              <span className="flex h-10 w-10 flex-none items-center justify-center rounded-xl bg-gold text-ink">
                <ShieldCheck className="h-5 w-5" />
              </span>
              <div>
                <p className="font-display font-bold text-white">Love it, or we make it right.</p>
          ...
          <Reveal direction="right">
            <ul className="border-y border-white/10">
              {FAQS.map((item, i) => {
                const isOpen = open === i;
                return (
                  <li key={item.q} className="border-b border-white/10 last:border-b-0">
                    <button
                      type="button"
                    ...
                    >
                      <div className="overflow-hidden">
                        <p className="pb-6 pr-10 text-white/55">{item.a}</p>
                      </div>
                    </div>
                  </li>
                );
